...
public:
    ...
    public:
        ...
        /**
         * @brief 迭代器的后置自增运算符++p\--P. 用于将迭代器指向下一个\前一个节点.
         *
         */
...
public:
    ...
    {
        Node *p_to= to.current->prev;
        Node *p_from = from.current;
        Node *p = p_from;
        Node  *q = nullptr;
        p_from->prev->next = p_to->next;
        p_to->next->prev = p_from->prev;
        for(p = p_from; p != to.current;)
        {
            q = p->next;
            delete p;
            p = q;
